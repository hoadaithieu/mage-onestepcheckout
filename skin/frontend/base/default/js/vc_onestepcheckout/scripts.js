...
//        //this.accordion.openNextSection(true);
    ...
    setStepResponse: function(response){
        if (response.update_section) {
			...
			if (response.update_section.name == 'shipping-method' || response.update_section.name == 'payment-method') {
				if (response.update_section.shipping_method_html) {
					$('checkout-shipping-method-load').update(response.update_section.shipping_method_html);
					$$('input[type="radio"][name="shipping_method"]').each(function(element){
				...
				if (response.update_section.payment_method_html) {
					$('checkout-payment-method-load').update(response.update_section.payment_method_html);
				}
				
				if (response.update_section.review_html) {
					$('checkout-review-load').update(response.update_section.review_html);
				}
...
Review.prototype = {
    ...
    save: function(){
        ...
        var selectElement = $('billing-address-select')
        if (!selectElement || (selectElement && selectElement.value == '')) {
        ...
        if (shippingMethod.validate()) {
			...
			params += '&'+Form.serialize(shippingMethod.form);
...
LoginForm.prototype = {
    ...
    save: function(){
        ...
        if (validator.validate()) {
			...
			$('vcosc-login-waiting').style.display = '';
    ...
    resetLoadWaiting: function(transport){
		$('vcosc-login-waiting').style.display = 'none';
    },
...
function toCenter(element, _x, _y, _p) {
	var elt         = $(element);
	...
	p = _p.length > 0 ? _p: 'absolute';
	// set the style of the element so it is centered
	var styles = { position : p,
		top      : y + 'px',
		left     : x + 'px' };
